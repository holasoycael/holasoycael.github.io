...
function related_results_labels_thumbs(json){
	for(var i=0;i<json.feed.entry.length;i++){
		var entry=json.feed.entry[i];relatedTitles[relatedTitlesNum]=entry.title.$t;
...
function printRelatedLabels_thumbs(current){
	...
	if(typeof(splittercolor)!=='undefined'){
		postNotFound=splittercolor;
	}
	else{
		postNotFound="undefined";
		}
		for(var i=0;i<relatedUrls.length;i++){
		...
		if(relatedTitles.length>0){
			document.write('<h2>'+relatedpoststitle+'</h2><div class="relatedPost">');
		}
		document.write('<div class="postRelated">');
		while(i<relatedTitles.length&&i<20&&i<maxresults){
			document.write('<li><a');
			if(i!=0)document.write('border-left:solid 0.5px '+postNotFound+';"');
			else document.write('"');
			document.write(' href="'+relatedUrls[r]+'"><img class="resizethumbnail" id="imagem" src="'+thumburl[r]+'"/><br/><div id="texto">'+relatedTitles[r]+'</div></a></li>');
			i++;
